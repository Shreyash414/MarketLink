"""
Task 2 — Real Potato ML Model Training, Feature Selection, Evaluation & Validation.
Executed strictly on genuine AGMARKNET history: data/raw/potato_agra_history.csv
"""
...
def run_potato_ml_pipeline() -> Dict[str, Any]:
    print("=== STARTING TASK 2: REAL POTATO ML PIPELINE ===")

    # ----------------------------------------------------
    # Step 2: Potato Data Profiling
    # ----------------------------------------------------
    raw_path = RAW_DATA_DIR / "potato_agra_history.csv"
    ...
    profile_df.to_csv(PROCESSED_DATA_DIR / "potato_data_profile.csv", index=False)
    ...
    # ----------------------------------------------------
    clean_model_path = PROCESSED_DATA_DIR / "potato_agra_model.csv"
    clean_path = PROCESSED_DATA_DIR / "potato_agra_clean.csv"
    ...
    v3_path = feats_dir / "potato_agra_features_v3.csv"
    ...
    splits_dir = PROCESSED_DATA_DIR / "splits_potato"
    ...
    models_potato_dir = PROCESSED_DATA_DIR / "models" / "potato"
    models_potato_dir.mkdir(parents=True, exist_ok=True)
    error_analysis_path = models_potato_dir / "error_analysis.csv"
    ...
    upward_spikes = int(((test_df["price_change"] >= spike_threshold)).sum())
    downward_spikes = int(((test_df["price_change"] <= -spike_threshold)).sum())
    ...
    # ----------------------------------------------------
    model_output_dir = get_model_dir(commodity="Potato", model_type="change_xgboost_v3")
    model_output_dir.mkdir(parents=True, exist_ok=True)

    model_path = model_output_dir / "agra_final_model.json"
    feature_path = model_output_dir / "agra_final_features.csv"
    ...
    register_model(
        ...
        model_file="agra_final_model.json",
        feature_file="agra_final_features.csv",
    ...
    cfg = get_commodity_config("Potato")
    cfg.status = "VALIDATED"
    cfg.model_status = "VALIDATED"
    cfg.training_eligible = True
    cfg.model_count = max(cfg.model_count, 1)
    cfg.default_markets = list({*cfg.default_markets, "Agra"})
    cfg.historical_mae["agra"] = float(round(test_mae, 2))
    cfg.notes = f"Genuine AGMARKNET historical model for Agra: MAE={round(test_mae, 2)}, R2={round(test_r2, 4)}"
    register_commodity_config(cfg)

    print(f"Step 14 Complete: Saved Potato model to {model_path} and registered in model_registry.json")

    # ----------------------------------------------------
    # Step 15: Inference Validation (ModelPredictor)
    # ----------------------------------------------------
    ...
    assert pred_res.predicted_price is not None, "ModelPredictor failed for Potato Agra"
    ...
    # ----------------------------------------------------
    # Step 16: Risk + Recommendation Compatibility
    # ----------------------------------------------------
    ...
    print(f"Step 16 Complete: MandiRecommender OK (Top Mandi: {rec_res.recommended_mandi}, Status={rec_res.status if hasattr(rec_res, 'status') else 'OK'})")
