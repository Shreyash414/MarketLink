"""
Task 5 — Real Rice ML Model Training, Feature Selection, Evaluation & Validation.
Executed strictly on genuine AGMARKNET history: data/raw/rice_burdwan_history.csv
"""
...
def run_rice_ml_pipeline() -> Dict[str, Any]:
    print("=== STARTING TASK 5: REAL RICE ML PIPELINE (BURDWAN, WB) ===")

    # ----------------------------------------------------
    # Step 3: Rice Data Profiling
    # ----------------------------------------------------
    raw_path = RAW_DATA_DIR / "rice_burdwan_history.csv"
    ...
    profile_df.to_csv(PROCESSED_DATA_DIR / "rice_data_profile.csv", index=False)
    print(f"Step 3 Complete: Profile saved ({total_rows} rows, {unique_dates} unique dates)")
    ...
    q_df.to_csv(PROCESSED_DATA_DIR / "rice_quality_report.csv", index=False)
    ...
    # ----------------------------------------------------
    clean_model_path = PROCESSED_DATA_DIR / "rice_burdwan_model.csv"
    clean_path = PROCESSED_DATA_DIR / "rice_burdwan_clean.csv"
    ...
    # ----------------------------------------------------
    # Step 7: V3 Feature Engineering (No Future Leakage)
    # ----------------------------------------------------
    ...
    v3_path = feats_dir / "rice_burdwan_features_v3.csv"
    ...
    splits_dir = PROCESSED_DATA_DIR / "splits_rice"
    splits_dir.mkdir(parents=True, exist_ok=True)

    train_df.to_csv(splits_dir / "rice_train.csv", index=False)
    val_df.to_csv(splits_dir / "rice_validation.csv", index=False)
    test_df.to_csv(splits_dir / "rice_test.csv", index=False)
    ...
    models_rice_dir = PROCESSED_DATA_DIR / "models" / "rice"
    models_rice_dir.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(val_selection_results).to_csv(models_rice_dir / "validation_feature_selection_results.csv", index=False)
    ...
    error_analysis_path = models_rice_dir / "error_analysis.csv"
    ...
    # ----------------------------------------------------
    model_output_dir = get_model_dir(commodity="Rice", model_type="change_xgboost_v3")
    model_output_dir.mkdir(parents=True, exist_ok=True)

    model_path = model_output_dir / "burdwan_final_model.json"
    feature_path = model_output_dir / "burdwan_final_features.csv"
    ...
    register_model(
        ...
        model_file="burdwan_final_model.json",
        feature_file="burdwan_final_features.csv",
    ...
    cfg = get_commodity_config("Rice")
    cfg.status = "VALIDATED"
    cfg.model_status = "VALIDATED"
    cfg.training_eligible = True
    cfg.model_count = max(cfg.model_count, 1)
    cfg.default_markets = list({*cfg.default_markets, "Burdwan"})
    cfg.historical_mae["burdwan"] = float(round(test_mae, 2))
    cfg.notes = f"Genuine AGMARKNET historical model for Burdwan: MAE={round(test_mae, 2)}, R2={round(test_r2, 4)}"
    register_commodity_config(cfg)

    print(f"Step 15 Complete: Saved Rice model to {model_path} and registered in model_registry.json")
    ...
    rec_res = recommender.recommend(
        farmer_latitude=23.2324,  # Burdwan coordinates
        farmer_longitude=87.8615,
        quantity_quintals=10.0,
        commodity="Rice",
    )
    print(f"Step 18 Complete: MandiRecommender Rice OK (Top Mandi: {rec_res.recommended_mandi}, Evaluated={rec_res.total_mandis_evaluated})")
